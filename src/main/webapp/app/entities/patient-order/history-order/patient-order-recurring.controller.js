(function () {
    ...
    angular
        .module('bluebookApp')
        .controller('RecurringHistoryOrderController', RecurringHistoryOrderController);

    RecurringHistoryOrderController.$inject = ['$compile', '$scope', '$state', 'PatientOrder', 'entity',
        '$q', 'DTColumnBuilder', 'DTOptionsBuilder', 'GUIUtils', '$filter', 'moment', '$parse', 'CoreService'];

    function RecurringHistoryOrderController($compile, $scope, $state, PatientOrder, entity,
                                      $q, DTColumnBuilder, DTOptionsBuilder, GUIUtils, $filter, moment, $parse, CoreService) {
        var vm = this;
        ...
        vm.dtOptions = DTOptionsBuilder.fromFnPromise(function () {
            ...
            if (!vm.searchQuery || vm.searchQuery == '') {
                PatientOrder.recurring_old({id : vm.patient.id}, function (result) {
                    vm.orders = result;
        ...
        function actionsHtml(data, type, full, meta){
            ...
            stButtons += '<a class="btn-sm btn-warning" ui-sref="patient-order-view({oid:' + full.id + '})">' +
